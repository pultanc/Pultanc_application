...
const copyLinkBlock = `          {/* Separate Copy Link Section */}
...
// find the line index where `<div className="absolute bottom-4 left-4 right-4` is
const index = lines.findIndex(line => line.includes('Click the download button to export'));
// we insert it after index + 2 (which is </div> and </div> of wrapper)
if (index !== -1) {
    lines.splice(index + 3, 0, copyLinkBlock);
}
